...
class CriminalEntity
  ...
  def make
    opts = @_entopts.dup
    CriminalEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PogonyHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Criminal, Hash] the current Criminal data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PogonyHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Criminal fields)
  def match_get
  ...
  #
  # @param reqmatch [CriminalListMatch, Hash, nil] match filter (any subset of
  #   Criminal fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Criminal>, Array] the matching Criminal items; raises PogonyError on failure
