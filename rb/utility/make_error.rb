...
module PogonyUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = PogonyContext.new({}, nil)
    end
    op = ctx.op || PogonyOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || PogonyResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(PogonyError) ? err.msg : err.to_s
    ...
    sdk_err = PogonyError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(PogonyError)
