# Pogony Golang SDK Reference

Complete API reference for the Pogony Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *PogonySDK`
...
#### `Criminal(data map[string]any) PogonyEntity`
...
fmt.Println(criminal.GetName()) // "criminal"
```
...
```go
results, err := client.Criminal(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(results)
