...
import (
	...
	omni "github.com/voxgig-sdk/pogony-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PogonyError); is {
				out := map[string]any{
					"name":    "PogonyError",
...
func omniCtx(arg any, client *sdk.PogonySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PogonySDK); is {
			client = live
