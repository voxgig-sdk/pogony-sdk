...
const TEST_SEED = { "entity": { "criminal": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+PogonySDK\b/.test(code) ||
        /\bPogonySDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+PogonySDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/PogonySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
