...
type Args struct {
	Entity string         `json:"entity" jsonschema:"criminal"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.PogonySDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "pogony_list",
		Description: "List records from Pogony. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "pogony_load",
		Description: "Load a single record from Pogony. " +
...
func runOp(client *sdk.PogonySDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.PogonySDK, name string) (sdk.PogonyEntity, error) {
	switch strings.ToLower(name) {
	case "criminal":
		return client.Criminal(nil), nil
