...
[MCP](https://modelcontextprotocol.io) server exposing the Pogony SDK as
two agent tools — `pogony_list` and `pogony_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pogony-mcp)
make build

# 2. Provide credentials via the environment
export POGONY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user pogony \
  -- /absolute/path/to/pogony-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./pogony-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// pogony_list: first page of records
{ "entity": "criminal" }
{ "entity": "criminal", "query": { } }

// pogony_load: one record by id
{ "entity": "criminal", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pogony-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user pogony \
     -- "$PWD"/dist/*/pogony-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `pogony_list` and `pogony_load` tools now appear
   in new sessions. Ask the agent to *"list criminal using pogony"*
   and it calls `pogony_list` with `{"entity":"criminal"}`.
...
```sh
export POGONY_APIKEY=sk_live_xxx            # API key
export POGONY_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `pogony_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `pogony_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `POGONY_APIKEY` | API key sent with every request. |
| `POGONY_BASE` | Optional override of the API base URL. |
...
```sh
./pogony-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"pogony_load","arguments":{"entity":"criminal","query":{"id":1}}}}'
```
