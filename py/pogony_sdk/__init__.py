...
from pogony_sdk.utility import register

# Load features
from pogony_sdk.feature.base_feature import PogonyBaseFeature
from pogony_sdk.features import _has_feature, _make_feature
...
class PogonySDK:
    ...
    def get_utility(self):
        return PogonyUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("PogonySDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def Criminal(self, data=None) -> "CriminalEntity":
        """Entity factory: client.Criminal().list() / client.Criminal().load({"id": ...})."""
        from pogony_sdk.entity.criminal_entity import CriminalEntity
        return CriminalEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "PogonySDK":
        if sdkopts is None:
            sdkopts = {}
