...
from pogony_sdk.pogony_types import (
    Criminal,
    CriminalListMatch,
)
...
class CriminalEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CriminalEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Criminal]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Criminal().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
