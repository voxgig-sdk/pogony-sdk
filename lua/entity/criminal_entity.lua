...
local CriminalEntity = {}
CriminalEntity.__index = CriminalEntity


function CriminalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CriminalEntity)
  self._name = "criminal"
  self._client = client
...
function CriminalEntity:get_name()
  return self._name
end


function CriminalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CriminalEntity.new(self._client, opts)
end
...
function CriminalEntity:mark_deleted()
  self._deleted = true
end


function CriminalEntity:deleted()
  return true == self._deleted
end


function CriminalEntity:data_set(args)
  if args ~= nil then
...
function CriminalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CriminalEntity:match_set(args)
  if args ~= nil then
...
function CriminalEntity:match_get()
...
function CriminalEntity:stream(action, args, callopts)
...
---@param reqmatch CriminalListMatch
---@param ctrl? table
---@return Criminal[]
---@return string? err
function CriminalEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CriminalEntity:_run_op(ctx, post_done)
  local utility = self._utility
