...
{
    private static function utility(): PogonyUtility
    {
        return new PogonyUtility();
    }
    ...
    private static function ctx(array $over = []): PogonyContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new PogonyContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new PogonyOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PogonyError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PogonyOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PogonyMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PogonyOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PogonyMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PogonyOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PogonyMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PogonySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PogonyMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new PogonyResponse([]),
            'result' => new PogonyResult([]),
        ]);
        [, $err] = PogonyMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PogonySpec(['step' => 's']),
            'response' => new PogonyResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PogonyResult([]),
        ]);
        [, $err] = PogonyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PogonySpec(['step' => 's']),
            'response' => new PogonyResponse(self::resp(200, ['v' => 1])),
            'result' => new PogonyResult([]),
        ]);
        [, $err] = PogonyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PogonySpec(['step' => 's']),
            'response' => new PogonyResponse(self::resp(200, ['v' => 2])),
            'result' => new PogonyResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PogonyMakeResponse::call($ctx);
    ...
    {
        $preset = new PogonyResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PogonySpec([]),
            'response' => new PogonyResponse([]),
            'result' => new PogonyResult([]),
        ...
        [$got, $err] = PogonyMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PogonyResult([])]);
        [, $err] = PogonyMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PogonySpec([]), 'result' => null]);
        [, $err] = PogonyMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PogonySpec(['step' => 's']),
            'result' => new PogonyResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PogonyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PogonyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PogonySpec(['step' => 's']),
            'result' => new PogonyResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PogonyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PogonyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PogonyResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PogonySpec([]),
            'result' => new PogonyResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PogonyMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PogonyMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new PogonyError('boom', 'boom');
        $utility->fetcher = function (PogonyContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PogonySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PogonyMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PogonyContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PogonyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PogonyResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PogonyContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PogonySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PogonyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PogonyResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PogonySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PogonyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = PogonyMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PogonyMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PogonyResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PogonyDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PogonyResult(['ok' => false])]);
        $this->expectException(PogonyError::class);
        PogonyDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PogonyResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PogonyMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PogonyBaseFeature();
        $b = new PogonyBaseFeature();
        PogonyFeatureAdd::call($ctx, $a);
        PogonyFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PogonyBaseFeature
    {
        $f = new PogonyBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        PogonyFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?PogonySpec $spec): PogonyContext
    {
    ...
    private static function auth_bags(): PogonySpec
    {
        return new PogonySpec(['headers' => [], 'query' => []]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PogonyPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new PogonyResponse(['headers' => null]),
            'result' => new PogonyResult([]),
        ]);
        PogonyResultHeaders::call($ctx);
