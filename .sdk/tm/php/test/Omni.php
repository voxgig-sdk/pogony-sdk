...
//    only - a typed PogonyContext there would make every ctx
...
final class PogonyOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new PogonyContext($clean, null);
    ...
    public static function err_from_map($m): ?PogonyError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new PogonyError(is_string($code) ? $code : '', $msg);
    }
