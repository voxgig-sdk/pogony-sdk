...
class PogonyMakeError
{
    public static function call(?PogonyContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new PogonyContext([], null);
        }
        $op = $ctx->op ?? new PogonyOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new PogonyResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof PogonyError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new PogonyError('', $msg, $ctx);
