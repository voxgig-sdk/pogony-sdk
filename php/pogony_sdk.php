...
class PogonySDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = PogonyHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = PogonyHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!PogonyFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, PogonyFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return PogonyUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = PogonyHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = PogonyHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = PogonyHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new PogonyError($op . "_allow",
                "PogonySDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = PogonyHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new PogonyError("graphql_error",
                "PogonySDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_criminal = null;

    // Canonical facade: $client->Criminal()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->criminal()
    // resolves here too.
    public function Criminal($data = null)
    {
        require_once __DIR__ . '/entity/criminal_entity.php';
        if ($data === null) {
            if ($this->_criminal === null) {
                $this->_criminal = new CriminalEntity($this, null);
            }
            return $this->_criminal;
        }
        return new CriminalEntity($this, $data);
    }
