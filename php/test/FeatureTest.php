...
class FtCtrl extends PogonyControl
{
...
class FtClient
{
    ...
    private PogonyUtility $utility;

    public function __construct(PogonyUtility $utility, array $options)
    {
    ...
    public function get_utility(): PogonyUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PogonyContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PogonyContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PogonyConfig::make_feature($name);
    ...
    private static function buildUrl(PogonySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PogonyError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PogonyResponse($fetched);
        }
    ...
    private function fail(PogonyContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PogonyContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
