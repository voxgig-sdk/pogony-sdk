...
class PogonyNetsimFeature extends PogonyBaseFeature
{
    ...
    private function _simulate(PogonyContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(PogonyContext $ctx, array $applied): void
    {
        $client = $this->client;
