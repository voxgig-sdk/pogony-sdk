...
class CriminalEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CriminalEntity($this->_client, $opts);
    }

    /**
     * @param Criminal|array $args Criminal data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = PogonyHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Criminal|array The current Criminal data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Criminal fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = PogonyHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Criminal fields).
     */
    ...
    /**
     * List Criminal items matching the given filter.
     *
     * @param CriminalListMatch|array|null $reqmatch Match filter (any subset
     *   of Criminal fields) as an assoc-array; CriminalListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Criminal[]|array A list of Criminal items as assoc-arrays at
     *   the SDK boundary; throws PogonyError on failure (item-5 convention).
     */
