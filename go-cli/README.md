...
boru-driven command-line client **and** interactive REPL for the Pogony
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pogony-cli)
make build
...
./pogony-cli list criminal

# 5. Override the API base URL for a single call
POGONY_BASE=https://api.example.com ./pogony-cli list criminal

# 6. No arguments -> interactive REPL
./pogony-cli
pogony> list criminal
pogony> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pogony-cli
   ```
...
Run with no arguments to open a REPL (prompt `pogony>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `criminal`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/pogony-cli`. |
...
(`./pogony-cli <expr>`) and interactively (the REPL), and expressions compose the
